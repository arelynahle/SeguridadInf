...
public class InstDAO {
	...
	public void setConnection(Connection connection){
	 ...
	 public void addInst (Institucion institucion)  {
         ...
         public void updateInst(Institucion institucion) {
        try {
            statement = connection.prepareStatement("UPDATE institucion SET nombre_inst=? ,calle_inst=? ,numero_inst=? ,colonia_inst=? ,cp_inst=? ,ciudad_inst=?, estado_inst=?, tel_inst=?, max_ninos_inst=?, nombre_dir=?, ap_dir=?, am_dir=?, cel_dir=?, email_dir=? WHERE id_inst =?");
            synchronized (statement) {
                statement.setString(1, institucion.getnombre_inst());
                statement.setString(2, institucion.getcalle_inst());
                statement.setInt(3, institucion.getnumero_inst());
                ...
                statement.setString(13, institucion.getcel_dir());
                statement.setString(14, institucion.getemail_dir());
                statement.setInt(15, institucion.getid_inst());
                statement.executeUpdate();
            }
            statement.close();
        } catch (SQLException sqle) {
		         ...
		         synchronized(statement) {
		            ...
		            while (results.next()) {
			           Institucion institucion = new Institucion();
                                   institucion.setid_inst(results.getInt("id_inst"));
        ...
        public Institucion searchInst(int id_inst) {
        Institucion institucion = null;
        try {
            statement = connection.prepareStatement("SELECT * FROM institucion WHERE id_inst = ?");
            statement.setInt(1, id_inst);
            // obtain user
            synchronized (statement) {
                ResultSet results = statement.executeQuery();
		            // get rows data{
